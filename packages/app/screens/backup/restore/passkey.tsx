...
export function RestoreBackupWithPasskeyForm(props: {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const error = parseError(err, localization.failedToRestore[language]);
      showSnackbar({
      ...
      <View className='bg-card mx-4 -mt-12 flex flex-col items-center space-y-2 rounded-[36px] px-4 py-6'>
        ...
        <View className='flex flex-col items-center'>
          <Text className='text-text-primary text-base font-medium'>
            {localization.restoreFromPasskey[language]}
          </Text>
          <Text className='text-text-secondary text-center text-sm font-normal'>
            {localization.unlockPasskey[language]}
          </Text>
        </View>
        <View className='w-full pt-4'>
          <TextButton
            text={localization.restore[language]}
            disabled={loading}
            loading={loading}
